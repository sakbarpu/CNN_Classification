...
[Wc, Wd, bc, bd] = cnnPtoS(theta,imageDim,filterDim,numFilters,...
                        poolDim,numClasses);
...
%%% YOUR CODE HERE %%%
% t_sum_exp = zeros(size(probs));
% for i = 1: size(probs,1)
%     
%     wtd = Wd(i,:);
%     t_sum_exp(i,:) = wtd * activationsPooled;
%     
% end
% 
% sum_exp = sum (t_sum_exp,2);
% sum_exp = exp (sum_exp);
% total_sum_exp = sum(sum_exp);
% 
% probs = t_sum_exp / total_sum_exp;
...
%%% YOUR CODE HERE %%%
% sparse_label = zeros(numClasses, size(images,3));
% 
...
% end
% 
% cost = -sum(sum(sparse_label - probs));
